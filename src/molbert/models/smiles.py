...
from molbert.tasks.tasks import MaskedLMTask, IsSameTask, PhyschemTask, InvitroTasks, InvivoTasks, BaseTask
...
class SmilesMolbertModel(MolbertModel):
    ...
    def get_config(self):
        if not hasattr(self.hparams, 'vocab_size') or not self.hparams.vocab_size:
            ...
            print("invitro tasks", self.hparams.num_invitro_tasks)
            print("invivo tasks", self.hparams.num_invivo_tasks)
        ...
        if self.hparams.tiny:
            ...
            print(self.hparams.vocab_size)
            config = BertConfigExtras(
                vocab_size_or_config_json_file=self.hparams.vocab_size,
                hidden_size= self.hparams.bert_output_dim,
                num_hidden_layers=12,
                num_attention_heads=12,
                intermediate_size=3072,
                max_position_embeddings=self.hparams.max_position_embeddings,
                num_physchem_properties=self.hparams.num_physchem_properties,
                num_invitro_tasks=self.hparams.num_invitro_tasks,
                num_invivo_tasks=self.hparams.num_invivo_tasks,
                invitro_head_hidden_layer = self.hparams.invitro_head_hidden_layer,
                invivo_head_hidden_layer = self.hparams.invivo_head_hidden_layer,
                named_descriptor_set=self.hparams.named_descriptor_set,
                is_same_smiles=self.hparams.is_same_smiles,
            )
    ...
    def get_tasks(self, config):
        ...
        if self.hparams.num_invitro_tasks > 0:
           tasks.append(InvitroTasks(name='invitro_tasks', config=config))

        if self.hparams.num_invivo_tasks > 0:
           tasks.append(InvivoTasks(name='invivo_tasks', config=config))
