...
from molbert.tasks.heads import PhysChemHead, invitro_head, invivo_head, FinetuneHead, IsSameHead
...
class InvitroTasks(BaseTask):
    def __init__(self, name, config):
        super().__init__(name)
        self.loss = MSELoss()

        self.invitro_head = invitro_head(config)

    def forward(self, sequence_output, pooled_output):
        return self.invitro_head(pooled_output)

    def compute_loss(self, batch_labels, batch_predictions) -> torch.Tensor:
        return self.loss(batch_predictions[self.name], batch_labels[self.name])
    
class InvivoTasks(BaseTask):
    def __init__(self, name, config):
        super().__init__(name)
        self.loss = MSELoss()

        self.invivo_head = invivo_head(config)

    def forward(self, sequence_output, pooled_output):
        return self.invivo_head(pooled_output)
