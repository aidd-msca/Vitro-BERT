...
#########################################################################################
class BertFinetuneSmilesDataset_MF(BaseBertDataset):
    def __init__(
        self,
        input_path,
        featurizer,
        ...
        **kwargs,
        ...
        #########################################
        self.labels = data[label_column].values
        self.drug_index = np.arange(0, self.labels.shape[0], 1)
        self.task_index = np.arange(0, self.labels.shape[1], 1)
    ...
    def prepare_sample(
        ...
        inputs, labels = super().prepare_sample(cur_id, encoded_tokens_a, None, is_same)
        labels['finetune'] = torch.tensor(self.labels[cur_id], dtype=torch.float).unsqueeze(0)
        drug_index = self.drug_index[cur_id]
        task_index = self.task_index

        labels['mol_indices'] = torch.tensor(drug_index, dtype = torch.long)
        labels['task_indices'] = torch.tensor(task_index, dtype = torch.long)
