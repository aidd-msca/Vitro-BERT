...
# config_dict
representation_dir = '/projects/home/mmasood1/arslan_data_repository/Tox21/BERT_representations/BERT_pretrained_on_Tox21_trainset/'
model_weights_dir = '/projects/home/mmasood1/Model_weights/Tox21/BERT/BERT_pretrainined_trainset/'
...
data_dir = '/projects/home/mmasood1/arslan_data_repository/Tox21/'
...
config_dict['project_name'] = "BERT_Tox21_pretraining"
config_dict['model_name'] = "Tox21_pretraining"
...
config_dict['train_file'] = data_dir + "Tox21_train_set.pkl"
config_dict['valid_file'] = data_dir + "Tox21_test_set.pkl"
config_dict['test_file'] = data_dir + "Tox21_test_set.pkl"
...
class MolbertModel(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        Masked_token_pred, Physchem_pred = self.forward(batch_inputs)
    ...
    def training_epoch_end(self, outputs):
        ...
        masking_loss = torch.stack([x['masking_loss'] for x in outputs]).mean()
        physchem_loss = torch.stack([x['physchem_loss'] for x in outputs]).mean()

        wandb.log({f'{step}_total_loss_epoch':total_loss.item()})
        wandb.log({f'{step}_masking_loss_epoch':masking_loss.item()})
        wandb.log({f'{step}_physchem_loss_epoch':physchem_loss.item()})

        # Log the learning rate at the end of each epoch
        lr = self.trainer.optimizers[0].param_groups[0]['lr']
        wandb.log({'learning_rate': lr})

        return {'{step}_loss':total_loss}
...
representaitons_path = config_dict['representation_dir'] + f"Tox21_BERT_features_pretrained_on_Tox21_trainset_epoch_{epoch}.csv"
...
if (os.path.exists(representaitons_path) == False) & (os.path.exists(model_weights) == True):
    ...
    Tox21 = pd.read_csv(config_dict['data_dir'] + "complete_Tox21_with_Normalized_SMILES.csv")
    SMILES = Tox21.SMILES.tolist()
    ...
    filtered_data = Tox21[filtered].reset_index(drop = True)
    ...
    filtered_data.to_csv(config_dict['representation_dir'] + f"Tox21_filtered_data.csv", index= False)
