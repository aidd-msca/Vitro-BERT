...
config_dict["pretrained_crash_model"] = "/projects/home/mmasood1/Model_weights/invitro/invitro_1million/MolBERT/Retrain_on_top_of_BERT/complete_1m_300k_ADME/ADME_masking_invitro_physchem_init_pretrained/epoch=19-step=0_v0.ckpt"
...
class MolbertModel(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        return {"loss": total_loss,
                ...
                "invitro_loss": Non_weighted_loss}
    ...
    def training_step_end(self, outputs):
        ...
        losses = {key: outputs[key].mean() for key in ['loss', 'masking_loss', 'physchem_loss','invitro_loss']}
        
        # Log the losses with WandB
        wandb.log({
            f'{step_prefix}total_loss_step': losses['loss'].item(),
            f'{step_prefix}masking_loss_step': losses['masking_loss'].item(),
            f'{step_prefix}physchem_loss_step': losses['physchem_loss'].item(),
            f'{step_prefix}invitro_loss_step': losses['invitro_loss'].item(),
    ...
    def training_epoch_end(self, outputs):
        ...
        'masking_loss': torch.stack([x['masking_loss'] for x in outputs]).mean().item(),
        'physchem_loss': torch.stack([x['physchem_loss'] for x in outputs]).mean().item(),
        'invitro_loss': torch.stack([x['invitro_loss'] for x in outputs]).mean().item()
    }
        
        # Log the losses with WandB
        wandb.log({
            f'{step_prefix}total_loss_epoch': losses['loss'],
            f'{step_prefix}masking_loss_epoch': losses['masking_loss'],
            f'{step_prefix}physchem_loss_epoch': losses['physchem_loss'],
            f'{step_prefix}invitro_loss_step': losses['invitro_loss'],
