...
# config_dict
model_weights_dir = '/projects/home/mmasood1/Model_weights/invitro/invitro_1million/MolBERT/Retrain_on_top_of_BERT/complete_1m_300k_ADME/BERT_pretraining_init_MolBERT_masking_physchem_head/'
...
config_dict['project_name'] = "BERT_pretraining_masking_physchem"
config_dict['model_name'] = "BERT_init_masking_physchem_head"
...
config_dict["invivo_batch_size"] = 0
...
config_dict["num_invitro_tasks"] = 0#len(target_names)
config_dict["num_of_tasks"] = 0#len(target_names)
...
class MolbertModel(pl.LightningModule):
    ...
    def forward(self, batch_inputs):
        ...
        return Masked_token_pred, Physchem_pred
    ...
    def training_step(self, batch, batch_idx):
        ...
        Masked_token_pred, _ = self.forward(corrupted_batch_inputs)
        _, Physchem_pred = self.forward(clean_batch_inputs)
        ...
        total_loss = masking_loss + physchem_loss
    ...
    def test_epoch_end(self, outputs):
        ...
        if self.hparams.gpu != -1:
            stepwise_loss = {key: torch.stack([output[key] for output in outputs]).detach().cpu().numpy() for key in outputs[0]}
        else:
            stepwise_loss = {key: torch.stack([output[key] for output in outputs]).mean(axis = 1).detach().cpu().numpy() for key in outputs[0]}
        stepwise_loss = pd.DataFrame(stepwise_loss)
        file_name = self.hparams.metadata_dir + f"training_loss_epoch_{self.hparams.epoch_number}_step_{self.hparams.step_number}.csv"
        #file_name = self.hparams.metadata_dir + f"val_loss_epoch_{self.hparams.epoch_number}_step_{self.hparams.step_number}.csv"

        stepwise_loss.to_csv(file_name, index = False)

        mean_losses = {key: torch.stack([output[key] for output in outputs]).mean().item() for key in outputs[0]}
...
####################################################################################33
config_dict["metadata_dir"] = "/projects/home/mmasood1/trained_model_predictions/SIDER_PreClinical/MolBERT/BERT_pretraining_init_MolBERT_masking_physchem_head/Training_loss/"
model_weights_path = "/projects/home/mmasood1/Model_weights/invitro/invitro_1million/MolBERT/Retrain_on_top_of_BERT/complete_1m_300k_ADME/BERT_pretraining_init_MolBERT_masking_physchem_head/"

#step_list = [0,567,1134,1701,2268,2835,3402,3969,4536,5103,5670]
config_dict["gpu"] = [1]
step_list = [0]
epoch_list = [16,17,18,19]
for step_number in step_list:
    for epoch_number in epoch_list:
        ...
        print(f"epoch {epoch_number}, step {step_number}")

        # load epoch epsific weights
        ...
        #checkpoint = torch.load(model_weights_path + f"epoch_0_step_{step_number}.ckpt", map_location=lambda storage, loc: storage)
        ...
        loss = trainer.test(model = model, 
                test_dataloaders = invitro_train_dataloader,
                #test_dataloaders = invitro_val_dataloader
                )
