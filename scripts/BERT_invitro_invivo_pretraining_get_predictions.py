...
# config_dict
#model_weights_dir = '/projects/home/mmasood1/Model_weights/invitro/invitro_1million/MolBERT/Retrain_on_top_of_BERT/complete_1m_300k_ADME/BERT_masking_physchem_invitro_invivo_pretraining_FL/'
...
invivo_pos_weights = "/projects/home/mmasood1/arslan_data_repository/invitro/invitro_1m/25_04_2024/pos_weights.csv"
...
config_dict['model_name'] = "BERT_masking_physchem_invitro_invivo_pretraining"
...
config_dict["pretrained_crash_model"] = None#"/projects/home/mmasood1/Model_weights/invitro/invitro_1million/MolBERT/Retrain_on_top_of_BERT/complete_1m_300k_ADME/ADME_masking_invitro_physchem_init_pretrained/epoch=0-step=0.ckpt"
...
class MolbertModel(pl.LightningModule):
    ...
    def extract_corrupted_cleaned_data(self, batch):
        ...
        if invivo_masked_inputs is not None:
            ...
            missing_invitro_labels = torch.full((invivo_labels.shape[0], 1234), -1, device = self.hparams.device)
            missing_invivo_labels = torch.full((invitro_labels.shape[0], 50), -1, device = self.hparams.device)
...
def extract_corrupted_cleaned_data(batch, device):
    invitro_clean, invivo_clean, invitro_masked, invivo_masked = batch[0], batch[1], batch[2], batch[3]
    (invitro_clean_inputs, _), _ = invitro_clean
    (invivo_clean_inputs, _), _ = invivo_clean
...
def get_model_predictions_MT(model, selected_dataloader, config):
    ...
    y_true_dict = {"invitro": [], "invivo": []}
    y_pred_dict = {"invitro": [], "invivo": []}


    for batch in tqdm(selected_dataloader):
        with torch.no_grad():
            batch_inputs_clean, batch_inputs_corrupted, batch_labels = extract_corrupted_cleaned_data(batch, device)
            _, _, invitro_pred, invivo_pred = model(batch_inputs_clean)

            for task in y_true_dict.keys():
                y_true_dict[task].append(batch_labels[task].squeeze().cpu())
            
            y_pred_dict["invitro"].append(invitro_pred.cpu())
            y_pred_dict["invivo"].append(invivo_pred.cpu())

    y_dict = {task: torch.cat(y_true_dict[task], dim=0) for task in y_true_dict.keys()}
    y_hat_dict = {task: torch.cat(y_pred_dict[task], dim=0) for task in y_pred_dict.keys()}

    y_dict["invitro"] = pd.DataFrame(y_dict["invitro"].cpu().detach().numpy(), columns=config["invitro_columns"])
    y_hat_dict["invitro"] = pd.DataFrame(y_hat_dict["invitro"].cpu().detach().numpy(), columns=config["invitro_columns"])

    y_dict["invivo"] = pd.DataFrame(y_dict["invivo"].cpu().detach().numpy(), columns=config["invivo_columns"])
    y_hat_dict["invivo"] = pd.DataFrame(y_hat_dict["invivo"].cpu().detach().numpy().reshape(-1, len(config["invivo_columns"])), columns=config["invivo_columns"])

    return y_dict["invitro"], expit(y_hat_dict["invitro"]), y_dict["invivo"], expit(y_hat_dict["invivo"])
...
preclinical_tasks = config_dict["invivo_columns"][:20]
clinical_tasks = config_dict["invivo_columns"][20:]
...
config_dict["metadata_dir"] = "/projects/home/mmasood1/trained_model_predictions/SIDER_PreClinical/MolBERT/BERT_masking_physchem_invitro_invivo_pretraining_FL_v2/"
...
#epoch_list = range(10)
step_list = [11820]
epoch_list = ["init_1"] #, "init"
#step_list = [0,  591,  1182, 1773, 2364, 2955, 3546, 4137, 4728, 5319, 5910]
#step_list = [0,  6501, 7092, 7683, 8274, 8865, 9456, 10047, 10638, 11229, 11820]
...
for epoch in epoch_list:
    for step in step_list:
        ...
        result_file = result_dir + f"invivo_test_metrics_epoch_{epoch}_step_{step}.csv"
        if epoch == "init":
            model_weights = config_dict['model_weights_dir'] + f"epoch_0_step_{step}.ckpt"
        elif epoch == "init_1":
            model_weights = config_dict['model_weights_dir'] + f"epoch_1_step_{step}.ckpt"
        else:
            model_weights = config_dict['model_weights_dir'] + f"epoch={epoch}-step={step}.ckpt"

        if (os.path.exists(result_file) == False) & (os.path.exists(model_weights) == True):
            ...
            invitro_labels, invitro_pred, invivo_labels, invivo_pred = get_model_predictions_MT(model, val_dataloader, config_dict)

            invitro_metrics = compute_binary_classification_metrics_MT(invitro_labels.values, invitro_pred.values, missing= -1)
            invivo_metrics = compute_binary_classification_metrics_MT(invivo_labels.values, invivo_pred.values, missing= -1)

            invitro_metrics.insert(0, 'Tasks', config_dict["invitro_columns"])    
            invivo_metrics.insert(0, 'Tasks', config_dict["invivo_columns"])    

            #metrics = metrics[metrics.Tasks != 'SIDER_binary']
            mean_preformances = {"pathology_mean": invivo_metrics[invivo_metrics.Tasks.isin(pathological_tasks)].iloc[:,1:].mean(),
                                            "blood_mean": invivo_metrics[invivo_metrics.Tasks.isin(blood_tasks)].iloc[:,1:].mean(),
                                            "preclinical_mean": invivo_metrics[invivo_metrics.Tasks.isin(preclinical_tasks)].iloc[:,1:].mean(),
                                            "clinical_mean": invivo_metrics[invivo_metrics.Tasks.isin(clinical_tasks)].iloc[:,1:].mean(),
                                            "combined_ex_BM":invivo_metrics[invivo_metrics.Tasks.isin(clinical_tasks + pathological_tasks)].iloc[:,1:].mean(),
                                            "combined_all": invivo_metrics.iloc[:,1:].mean()}
            
            mean_preformances = pd.DataFrame(mean_preformances).T
            mean_preformances = mean_preformances.rename_axis('Tasks').reset_index()
            invivo_metrics = pd.concat([invivo_metrics, mean_preformances], ignore_index=True) 

            # store obs, pred, results
            invitro_labels.to_csv(data_dir + f'invitro_obs_epoch{epoch}_step_{step}.csv', index=False)
            invitro_pred.to_csv(data_dir + f'invitro_pred_epoch{epoch}_step_{step}.csv', index=False)
            invivo_labels.to_csv(data_dir + f'invivo_obs_epoch{epoch}_step_{step}.csv', index=False)
            invivo_pred.to_csv(data_dir + f'invivo_pred_epoch{epoch}_step_{step}.csv', index=False)

            invitro_metrics.to_csv(result_dir + f'invitro_test_metrics_epoch_{epoch}_step_{step}.csv', index=False)
            invivo_metrics.to_csv(result_dir + f'invivo_test_metrics_epoch_{epoch}_step_{step}.csv', index=False)