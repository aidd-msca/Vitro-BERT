...
config_dict["invivo_batch_size"] = 256
...
class MolbertModel(pl.LightningModule):
    ...
    def test_epoch_end(self, outputs):
        ...
        #file_name = self.hparams.metadata_dir + f"{self.hparams.dataloader_name}_epoch_{self.hparams.epoch_number}.csv"
...
config_dict["metadata_dir"] = "/projects/home/mmasood1/trained_model_predictions/SIDER_PreClinical/MolBERT/BERT_pretraining_init_MolBERT_masking_head/embeddings/"
...
#epoch_start = 15
#for epoch_number in range(epoch_start,epoch_start + 5):
    for step_number in step_list:
        ...
        for i, d_loader in enumerate(data_loader_list):
            ...
            print(epoch_number, step_number, config_dict["dataloader_name"])


            # load epoch specific weights
            model = MolbertModel(config_dict)
            checkpoint = torch.load(model_weights_path + f"epoch={epoch_number}-step=0.ckpt", map_location=lambda storage, loc: storage)
