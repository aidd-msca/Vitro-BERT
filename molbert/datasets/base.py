...
class BaseBertDataset(Dataset, ABC):
    ...
    def load_sequences(sequence_file):
        ...
        #########################################
        if sequence_file.endswith('.csv'):
            data = pd.read_csv(sequence_file)
        elif sequence_file.endswith('.pkl'):
            data = pd.read_pickle(sequence_file)
    ...
    def __getitem__(self, index):
        ...
        ###########################33
        #self.smiles = self.sequences[index]
        ############################
    ...
    def prepare_sample(
        ...
        ####### clean sequence #################
        clean_features = convert_example_to_features(
            cur_example, self.total_seq_len, self.featurizer, self.original_smiles, inference_mode = True)
        # create final tensor
        clean_inputs = dict(
            input_ids=torch.tensor(clean_features.input_ids, dtype=torch.long),
            token_type_ids=torch.tensor(clean_features.segment_ids, dtype=torch.long),
            attention_mask=torch.tensor(clean_features.input_mask, dtype=torch.long))

        ####### corrupted sequence #################
        cur_features = convert_example_to_features(
            cur_example, self.total_seq_len, self.featurizer, self.original_smiles, inference_mode = False)
        # create final tensor
        ...
        return (corrupted_inputs, clean_inputs), labels
